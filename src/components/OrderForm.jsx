...
import { CalendarOutlined, PlusOutlined, MinusCircleOutlined } from "@ant-design/icons";
...
const OrderForm = () => {
  ...
  const [categories, setCategories] = useState([{ key: Date.now(), items: [{ key: Date.now(), value: "", quantity: 1 }], selectedCategory: "" }]);
  ...
  const handleCategoryChange = (key, selectedCategory) => {
    setCategories(categories.map(category => category.key === key ? { ...category, selectedCategory, items: [{ key: Date.now(), value: "", quantity: 1 }] } : category));
  };

  const addCategory = () => setCategories([...categories, { key: Date.now(), items: [{ key: Date.now(), value: "", quantity: 1 }], selectedCategory: "" }]);

  const removeCategory = (categoryKey) => {
    if (categories.length > 1) {
      setCategories(categories.filter(category => category.key !== categoryKey));
    }
  };

  const addOrderItem = (categoryKey) => setCategories(categories.map(category => category.key === categoryKey ? { ...category, items: [...category.items, { key: Date.now(), value: "", quantity: 1 }] } : category));

  const removeOrderItem = (categoryKey, itemKey) => setCategories(categories.map(category => category.key === categoryKey ? { ...category, items: category.items.filter(item => item.key !== itemKey) } : category));

  const handleOrderItemChange = (categoryKey, itemKey, value) => setCategories(categories.map(category => category.key === categoryKey ? { ...category, items: category.items.map(item => item.key === itemKey ? { ...item, value } : item) } : category));

  const handleQuantityChange = (categoryKey, itemKey, quantity) => setCategories(categories.map(category => category.key === categoryKey ? { ...category, items: category.items.map(item => item.key === itemKey ? { ...item, quantity } : item) } : category));
  ...
  const handleSubmit = (values) => {
    const templateParams = {
      ...
      categories: categories.map(category => ({
        ...
        items: category.items.map(item => `${item.value} (x${item.quantity})`).join(", "),
      })),
      ...
      content: (
        <div>
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please enter a contact phone number!" },
              { pattern: /^\d{3}-\d{3}-\d{4}$/, message: "Please enter a valid phone number!" },
            ]}
            className="mb-2"
          >
            <Input placeholder="Enter contact phone number" type="tel" className="w-full" onChange={handlePhoneNumberChange} />
          </Form.Item>
        </div>
      ),
    },
          ...
          >
            <div className="w-full flex gap-2">
              <Select
                ...
                value={selectedDate ? moment(selectedDate).format("MMMM D, YYYY") : undefined}
                className="w-full"
              >
              ...
              <Button icon={<CalendarOutlined />} onClick={() => setShowCalendar(!showCalendar)}></Button>
            </div>
          </Form.Item>
          {showCalendar && (
          ...
          {categories.map((category, categoryIndex) => (
            <div key={category.key} className="mb-4 border p-4 rounded">
                  ...
                  category.items.map((item, itemIndex) => (
                    ...
                    >
                      <Form.Item
                        ...
                        rules={[{ required: true, message: `Please select a ${category.selectedCategory}!` }]}
                        className="mb-2 w-full"
                      >
                        <Select
                          ...
                          className="w-full"
                          value={item.value}
                          onChange={(value) => handleOrderItemChange(category.key, item.key, value)}
                        >
                          {cateringMenu[category.selectedCategory.toLowerCase()].map((menuItem) => (
                      ...
                      <Form.Item label="Amount" className="mb-2">
                        <Input
                          ...
                          min="1"
                          value={item.quantity}
                          onChange={(e) => handleQuantityChange(category.key, item.key, parseInt(e.target.value))}
                          className="w-20"
                        />
                      </Form.Item>
                      ...
                      {category.items.length > 1 && (
                        <MinusCircleOutlined
                          className="text-red-500 cursor-pointer"
                          onClick={() => removeOrderItem(category.key, item.key)}
                        />
                      )}
                    </motion.div>
                  ))}
              ...
              <Form.Item className="mb-4">
                ...
                >
                  Add Another {category.selectedCategory.charAt(0).toUpperCase() + category.selectedCategory.slice(1)}
                </Button>
              </Form.Item>
            </div>
          ))}
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-2 items-center justify-center w-full">
          {renderSummary()}
          <Form.Item name="comments" label="Additional Comments" className="mb-2 w-full flex flex-col">
            <TextArea placeholder="Enter any additional details here" className="w-full" rows={4} />
          </Form.Item>
          <Button type="primary" htmlType="submit" className=" bg-mGreen text-white">
            Submit
          </Button>
        </div>
      ),
      ...
      <div className="p-2">
        ...
        >
          <div className="w-full overflow-x-auto h-[25%]">
            <Steps current={currentStep} className="w-full px-4" direction="horizontal">
          ...
          <Form form={form} onFinish={handleSubmit} className="w-full h-[75%] flex flex-col items-center justify-start px-4">
            {steps[currentStep].content}
            <div className="flex justify-center gap-4 mt-2 w-full">
              {currentStep > 0 && (
                <Button type="default" onClick={prev} className="bg-red-500 text-white">
                  Previous
                </Button>
              )}
